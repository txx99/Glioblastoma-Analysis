...
To work with limaa DEA, we first need a design matrix. We made a design matrix WITHOUT INTERCEPT (each group gets its own column). The intercept is essentially the "reference" (or baseline) group, from which we will base the comparisons on (example: evaluate how different group B is compared to group A). 
