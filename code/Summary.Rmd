...
First we chose the desired phenotypic characteristics to be used for analysis (disease state: PD or psPD). We then cleaned up the content of that column by removing the "disease state: " text and retaining only the actual diagnostic, which is then confirmed with table().
```{r data_cleanup}
...
```
The variable 'diagnosis' is assigned as a list for downstream use.
